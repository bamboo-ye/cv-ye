...
```
├── index.html          # 简历主页
├── resume.html         # 旧简历入口，自动跳转到主页
├── styles.css          # 历史样式文件（主页样式在 index.html 中）
├── rare/叶晓艺简历.pdf  # 简历 PDF 文件
├── .gitignore          # Git 忽略文件
└── README.md           # 项目说明文件
```
...
3. **更新简历 PDF**
   - 将新的简历 PDF 文件命名为 `叶晓艺简历.pdf`，替换 `rare/叶晓艺简历.pdf`
   - 同步更新 `index.html` 中两个下载链接的版本参数，避免缓存旧版 PDF（当前为 `v=20260925`）

4. **修改样式**
   - 打开 `index.html` 文件中的 `<style>` 部分
   - 根据需要修改颜色、字体、布局等样式
